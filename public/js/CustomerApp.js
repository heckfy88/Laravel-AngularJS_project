let app1 = angular.module('CustomerApp', [ 'ngRoute' ]);
app1.config(function configure($routeProvider) {
    $routeProvider
...
app1.factory('Data', function Data($http) {
    return {
...
app1.controller('CustomersController', function CustomersController($scope, Data) {
    ...
    $scope.addCustomer = function addCustomer() {
        let names = $scope.newCustomer.name.split(' ');
        Data.addCustomer({
            first_name: names[0],
            last_name: names[1],
...
app1.controller('CustomerController', function CustomerController($scope, $routeParams, Data) {
    ...
    function transactionRemoveSuccess(data) {
        let i = $scope.transactions.length;
        while (i--) {
